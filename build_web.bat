...
if "%EMSCRIPTEN_SDK_DIR%"=="" (	
	if "%1"=="" (		
		echo "EMSCRIPTEN_SDK_DIR not defined"		
		:: Point this to where you installed emscripten.
		set EMSCRIPTEN_SDK_DIR=c:\SDK\emsdk
	) else (
		set EMSCRIPTEN_SDK_DIR="%1"
	)
)
...
set files=%OUT_DIR%\gamegame.wasm.o %ODIN_PATH%\vendor\raylib\wasm\libraylib.a %ODIN_PATH%\vendor\raylib\wasm\libraygui.a

:: index_template.html contains the javascript code that calls the procedures in
:: source/main_web/main_web.odin
set flags=-sUSE_GLFW=3 -sWASM_BIGINT -sINITIAL_MEMORY=536870912 -sWARN_ON_UNDEFINED_SYMBOLS=0 -sASSERTIONS --shell-file source\main_web\index_template.html --preload-file assets
